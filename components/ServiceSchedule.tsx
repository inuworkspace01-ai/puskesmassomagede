...
const hours: Record<number, [string, string, string]> = {
  ...
  3: ['Rabu', '07.30', '12.00'], 4: ['Kamis', '07.30', '12.00'], 5: ['Jumat', '07.30', '11.00'], 6: ['Sabtu', '07.30', '12.00']
};

type Row = { id: string; title: string; category: string; day: string; time: string; staff: string; notes: string };

function parts(date: Date) {
  const f = new Intl.DateTimeFormat('en-US', { timeZone: 'Asia/Jakarta', weekday: 'short', hour: '2-digit', minute: '2-digit', hour12: false }).formatToParts(date);
...
export default function ServiceSchedule() {
  ...
  useEffect(() => {
    let alive = true;
    const load = async () => { try { const r = await fetch('/api/schedule', { cache: 'no-store' }); const d = await r.json(); if (alive && r.ok) setRows(d.items || []); } catch {} };
    load(); const refresh = setInterval(load, 30000); return () => { alive = false; clearInterval(refresh); };
  }, []);

  const x = parts(now); const s = hours[x.day]; const cur = x.hour * 60 + x.minute;
  const open = s[1] !== 'Tutup' && cur >= clockMinutes(s[1]) && cur < clockMinutes(s[2]);
  const date = new Intl.DateTimeFormat('id-ID', { timeZone: 'Asia/Jakarta', weekday: 'long', day: '2-digit', month: 'long', year: 'numeric' }).format(now);
  ...
  const todayLabel = new Intl.DateTimeFormat('id-ID', { timeZone: 'Asia/Jakarta', weekday: 'long', day: 'numeric', month: 'long', year: 'numeric' }).format(now).toLowerCase();
  ...
  return <div className="scheduleWidget">
    ...
    <div className="scheduleOperational">
      ...
      {todayRows.length ? <div className="operationalGrid">{todayRows.map(r => <div className="operationalCard" key={r.id}><div className="icon">{icon(r.category)}</div><div><div className="newsMeta"><span>{r.category}</span></div><h4>{r.title}</h4><p><b>{r.time}</b></p>{r.staff && <p>Petugas: <b>{r.staff}</b></p>}{r.notes && <small>Tugas: {r.notes}</small>}</div></div>)}</div> : <div className="emptyState">Belum ada jadwal perawat untuk hari ini.</div>}
    </div>
    <div className="scheduleList">{[1,2,3,4,5,6].map(d => <div key={d}><span>{hours[d][0]}</span><b>{hours[d][1]} – {hours[d][2]}</b></div>)}</div>
  </div>;
}