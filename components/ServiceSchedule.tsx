...
const min = (s: string) => { const p = s.split(':').map(Number); return p[0] * 60 + p[1]; };
...
export default function ServiceSchedule() {
  ...
  const open = s[1] !== 'Tutup' && cur >= min(s[1]) && cur < min(s[2]);
  ...
  const todayLabel = new Intl.DateTimeFormat('id-ID', { timeZone: 'Asia/Jakarta', weekday: 'long', day: 'numeric', month: 'long', year: 'numeric' }).format(now);
  const todayRows = useMemo(() => rows.filter(r => r.day.toLowerCase() === todayLabel.toLowerCase()), [rows, todayLabel]);
