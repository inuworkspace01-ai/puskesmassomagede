...
export async function ensureDatabase() {
  ...
  initialized = (async () => {
    ...
    if (Number(existing[0]?.count || 0) === 0) {
      await sql`
        INSERT INTO site_content (id,title,description,category,image_url,status,published_at)
        VALUES
          ('seed-tbc-ckg','Jadwal tracing TBC & integrasi CKG','Informasi kegiatan lapangan dan integrasi CKG di wilayah kerja Puskesmas Somagede.','Kegiatan','', 'published', NOW()),
          ('seed-germas','Edukasi GERMAS: hidup aktif dan sehat','Ayo biasakan aktivitas fisik, konsumsi pangan sehat, dan cek kesehatan secara berkala.','Edukasi','', 'published', NOW())
      `;
    }
    const schedules = await sql`SELECT COUNT(*)::int AS count FROM site_schedules`;
    if (Number(schedules[0]?.count || 0) === 0) {
      const rows = [
        ['sched-rujukan','Rujukan & poli','Rujukan / Poli','Sesuai pelayanan','Belum diatur','Belum diatur','Hubungi petugas untuk jadwal dan alur rujukan.'],
        ['sched-vk-mtbs','Pelayanan VK / MTBS','VK / MTBS','Sesuai jadwal','Belum diatur','Belum diatur','Jadwal petugas diisi melalui dashboard admin.'],
        ['sched-bidan-pagi','Bidan jaga pagi','Bidan Jaga','Pagi','Belum diatur','Belum diatur','Nama petugas diisi melalui dashboard admin.'],
        ['sched-bidan-sore','Bidan jaga sore','Bidan Jaga','Sore','Belum diatur','Belum diatur','Nama petugas diisi melalui dashboard admin.'],
        ['sched-bidan-malam','Bidan jaga malam','Bidan Jaga','Malam','Belum diatur','Belum diatur','Nama petugas diisi melalui dashboard admin.'],
